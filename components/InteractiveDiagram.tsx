...
export const InteractiveDiagram: React.FC<InteractiveDiagramProps> = ({ data }) => {
  ...
  useEffect(() => {
    ...
    node.append("rect")
      ...
      .attr("fill", (d: any) => d.id === selectedElement?.id ? "#e0f2fe" : "#f3f4f6");

    node.append("text")
      .text((d: any) => d.label)
      .attr("text-anchor", "middle").attr("dy", ".3em")
      .attr("font-size", "10px").attr("fill", "#111827");
    ...
    const drag = d3.drag()
        .on("start", (event: any, d: any) => { if (!event.active) simulationRef.current.alphaTarget(0.3).restart(); d.fx = d.x; d.fy = d.y; })
        .on("drag", (event: any, d: any) => { d.fx = event.x; d.fy = event.y; })
        .on("end", (event: any, d: any) => { if (!event.active) simulationRef.current.alphaTarget(0); d.fx = null; d.fy = null; });
    node.call(drag);
  ...
  }, [data, componentId, isDiagramFullscreen]);
  ...
  const InfoCard: React.FC = () => {
    ...
    return (
      <div className="absolute bottom-2 left-2 right-2 sm:left-auto sm:w-64 bg-white/80 backdrop-blur-sm rounded-lg shadow-lg border border-gray-200 p-3 text-xs z-10 animate-fade-in">
        <p className="font-bold text-gray-800">{selectedElement.label}</p>
        {selectedElement.description && <p className="text-gray-600 mt-1">{selectedElement.description}</p>}
      </div>
    );
  };
  
  return (
    <div ref={containerRef} className={`relative w-full h-full bg-white group ${isDiagramFullscreen ? 'fixed inset-0 z-40' : ''}`}>
